    ...
    for (const [nameKey, bundle] of merged.entries()) {
      ...
      await this.prisma.playerShotProfile.create({
        data: {
          ...
          gameDate: snapshotDate,
          minutes,
          catchShootAttempts: numberField(catchShoot, ['catch_shoot_fga', 'fga']),
          catchShootFrequency: numberField(catchShoot, ['freq', 'frequency']),
          catchShootEfficiency: firstDefinedNumber(catchShoot, ['efg_pct', 'fg_pct', 'catch_shoot_efg_pct']),
          pullupAttempts: numberField(pullUp, ['pull_up_fga', 'fga']),
          pullupFrequency: numberField(pullUp, ['freq', 'frequency']),
          pullupEfficiency: firstDefinedNumber(pullUp, ['efg_pct', 'fg_pct', 'pull_up_efg_pct']),
          expectedEfg: 0,
          source: 'stats.nba.com_tracking_snapshot',
          sourceTier: 'TIER_1_OFFICIAL',
        },
